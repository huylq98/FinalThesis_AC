...
/**
 * A Mapper can apply a regex substitution (typically removing 'redundant'
 * stuff) to a string, while retaining the true offsets of things. The
 * data-structure is a simple array with the format displacement n
 * 
 * @author mfreire
 */
public class Mapper {
	...
	private void buildDisplacements(String from, String to) {
		...
		while (m.find(prev)) {
			...
			if (matchLen != toLen) {
				ms.add(new Mapping(m.start(), m.end(), out.length(), out.length() + toLen));
			}
			out.append(to);
			prev = m.end();
		}
	...
	/**
	 * Maps an offset in the source document to an offset in the destination
	 * document. If the offset falls within a 'mapping', either the start
	 * ('biasForLow') or the end of the mapped mapping will be returned.
	 * 
	 * @param offset
	 * @param biasForLow if true, and within a mapped segment, start of segment will
	 *                   be returned.
	 * @return
	 */
	...
	/**
	 * Maps an offset in the destination document to an offset in the source
	 * document. If the offset falls within a reversed 'mapping', either the start
	 * ('biasForLow') or the end of the reversed mapped mapping will be returned.
	 * 
	 * @param offset
	 * @param biasForLow if true, and within a mapped segment, start of segment will
	 *                   be returned.
	...
	public int rmap(int offset, boolean biasForLow) {
		if (offset < 0 || offset > dest.length()) {
			throw new IllegalArgumentException("out of bounds: " + offset + " not inside [ 0, " + dest.length() + "]");
		}
	...
	/**
	 * describes, for two offsets in an origin space (a1, a2), the matching offsets
	 * in a target space (b1, b2)
	 */
