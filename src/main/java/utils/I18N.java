...
public class I18N {
	...
	private static final String messageFileNames[] = new String[] { "i18n/messages", "i18n/ui-messages" };
			...
			try {
				...
				for (Object k : all.keySet()) {
					...
					if (FileUtils.resourceExists(fileName)) {
						available.add(new Lang("" + k, all.getProperty("" + k)));
					} else {
						log.warn("Referenced in " + languageIndex + " but not found: " + fileName);
					}
				}
	 ...
	 *
	 * @param lang ISO-639 language code; null or any other not-found value will be
	 *             interpreted as the default language
	 */
	public static void setLang(String lang) {
		...
		try {
			...
			String langWithoutCountry = (first == -1) ? lang : lang.substring(0, first);
			...
			log.warn("Loaded all messages (" + I18N.messages.size() + " total); lang is " + lang);
		} catch (IOException ioe) {
	...
	/**
	 * Overlays current messages with more-specific variants. The previous messages
	...
	public static String m(String key, Object... args) {
		...
		if (end == -1) {
			if (args.length != 0) {
				log.warn("Extra args passed to " + key + ": " + Arrays.toString(args) + " but none expected in '"
						+ result + "'");
			}
			return result;
		} else if (args.length == 0) {
			log.warn("No args passed to " + key + ": but expected at least 1 in '" + result + "'");
			return result;
		}
		...
		do {
			if (currentArg == args.length) {
				...
				log.warn("Substitution requested in key " + key + " " + " but not enough args " + Arrays.toString(args)
						+ " provided for message '" + result + "'; recycling args");
				currentArg = 0;
			}
			replaced.append(result.substring(start, end)).append(args[currentArg]);
			currentArg++;
			substitutions++;
			start = end + "{}".length();
		...
		if (currentArg != args.length) {
			log.warn("Bad number of args (" + args.length + ") passed to " + key + ": expected " + substitutions
					+ " args for '" + result + "'");
		}
	...
	public static String m(int cardinality, String keyOne, String keyMany, Object... args) {
